...
function loadImage(result) {
    ...
    if (result === 'win') {
        ...
        <p class="image">Hooray!! That's right, the answer is <b>${rightAnswer}</b></p><br>
        <img class="gif" src="${correct[count]}"/>
        `)
    } else {
        $("#question-answer").html(`
        <p class="image">You picked the wrong answer! The answer is <b>${rightAnswer}</b></p><br>
        <img class="gif" src="${incorrect[count]}"/>
        `)
    }
